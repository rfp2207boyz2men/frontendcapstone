        ...
        <table>
          <tbody className="modalTBody">
            ...
            {featuresList.map(feature => {
              return <tr>
                      <td className="modalData">{current.features.filter(ft => ft.feature === feature.feature && ft.value === feature.value).length ? <FcCheckmark fontSize="16px"/>: null}</td>
                      <td className="modalData">{feature.value ? <div>{feature.feature}: {feature.value}</div> : <div>{feature.feature}</div>}</td>
                      <td className="modalData">{clicked.features.filter(ft => ft.feature === feature.feature && ft.value === feature.value).length ? <FcCheckmark fontSize="16px"/>: null}</td>
                     </tr>
            })}
          </tbody>
        </table>
